import React, { useEffect, useState } from 'react';
      ...
      // Day plan card: shows workouts and diet for the selected day
      const DayPlanCard = () => {
        ...
        // Force English weekday names to match backend values (e.g., "Monday")
        ...
        useEffect(() => {
          const fetchWorkouts = async () => {
            ...
            try {
              ...
              if (!username) {
                setWeeklyPlan([]);
                setError('No username found in local storage. Please log in again.');
                return;
              }
              // Use Vite proxy to avoid CORS: vite.config.js maps "/workouts" -> http://localhost:8083
              const res = await axios.get(`/workouts/get/${username}`);
              setWeeklyPlan(Array.isArray(res.data?.weeklyPlan) ? res.data.weeklyPlan : []);
            } catch (err) {
              console.error('Failed to fetch workouts', err);
              setWeeklyPlan([]);
              setError(err?.response?.data?.message || err?.message || 'Could not load your workout plan. Please try again shortly.');
            } finally {
              setLoading(false);
            }
          };
        ...
        const todayWorkout = weeklyPlan.find((w) => w?.dayOfWeek === dayOfWeek);
        ...
        const Chip = ({ children }) => (
          <span className="inline-flex items-center px-3 py-1 rounded-full bg-gray-100 text-gray-700 text-xs font-medium mr-2 mb-2">
            {children}
          </span>
        );
          ...
          <div className="bg-white rounded-3xl p-6 shadow-lg border border-gray-100">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-lg font-bold text-gray-800">{formatHuman(selectedDate)}</div>
                </div>
                <span className="ml-2 px-2 py-1 text-xs rounded-lg bg-emerald-50 text-emerald-700 border border-emerald-100">
                  {dayOfWeek}
                </span>
              </div>
            ...
            {loading ? (
              ...
              todayWorkout.restDay ? (
                <div className="rounded-2xl p-6 bg-gradient-to-br from-gray-50 to-gray-100 border border-gray-200 text-center">
                  <div className="text-4xl mb-2">🛌</div>
                  <div className="text-lg font-semibold text-gray-800 mb-1">Rest Day</div>
                  <div className="text-sm text-gray-600 mb-4">Recover and recharge. Light movement is still great!</div>
                  <div className="flex flex-wrap justify-center gap-2">
                    <Chip>15-min walk</Chip>
                    <Chip>Full-body stretch</Chip>
                    <Chip>Hydrate well</Chip>
                  </div>
                </div>
              ) : (
                <div>
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <span className="font-semibold text-gray-800">{todayWorkout.workoutType}</span>
                    <span className="px-2 py-0.5 text-xs rounded-md bg-orange-50 text-orange-700 border border-orange-100 ml-2">
                      {dayOfWeek}
                    </span>
                  </div>

                  <SectionTitle title="Warm-up" />
                  {Array.isArray(todayWorkout.warmUp) && todayWorkout.warmUp.length > 0 ? (
                    ...
                    <div className="text-sm text-gray-500 mb-2">No warm-up listed.</div>
                  )}
                  ...
                  {Array.isArray(todayWorkout.exercises) && todayWorkout.exercises.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {todayWorkout.exercises.map((e) => (
                        <div
                          key={e.id}
                          className="p-4 rounded-2xl border border-gray-200 hover:border-orange-200 bg-white hover:bg-orange-50/40 transition-colors"
                        >
                          ...
                          <div className="flex items-center gap-2 text-xs">
                            ...
                            <span className="px-2 py-1 rounded-md bg-sky-50 text-sky-700 border border-sky-100">
                              {e.reps}
                            </span>
                          </div>
                        </div>
                    ...
                    <div className="text-sm text-gray-500 mb-2">No exercises listed.</div>
                  )}

                  <SectionTitle title="Cool-down" />
                  {Array.isArray(todayWorkout.coolDown) && todayWorkout.coolDown.length > 0 ? (
                    <div className="flex flex-wrap">
                    ...
                    <div className="text-sm text-gray-500">No cool-down listed.</div>
                  )}
                </div>
              )
            ) : (
              <div className="text-center text-gray-500 py-8">No workout data for this day.</div>
            )}
          </div>
        );
      };
      ...
      const MetricCard = ({ title, value, unit, icon: Icon, progress, goal, bgGradient, emoji, encouragement }) => (
        <div
          className={`group relative overflow-hidden rounded-3xl p-4 sm:p-6 text-white shadow-lg hover:shadow-2xl transform hover:scale-105 transition-all duration-300 cursor-pointer ${bgGradient}`}
        >
          ...
          <div className="relative z-10">
              ...
              <div className="mb-4">
                ...
                <div className="flex items-center gap-2">
                  <div className="flex-1 bg-white/30 rounded-full h-2">
                    <div className="h-2 bg-white rounded-full transition-all duration-1000 ease-out" style={{ width: `${Math.min(progress, 100)}%` }}></div>
                  </div>
            ...
            {encouragement && <div className="text-xs opacity-90 font-medium bg-white/20 rounded-lg px-2 py-1 inline-block">{encouragement}</div>}
          </div>
        </div>
      );
      ...
      const Dashboard = () => {
        ...
        useEffect(() => {
          const fetchData = async () => {
            try {
              const username = localStorage.getItem('username');
              const response = await axios.get(`http://localhost:8084/user-details/get/${username}`);
            ...
            <div className="min-h-screen transition-all duration-300">
              ...
              <div className="flex flex-col xl:flex-row gap-6 p-4 lg:p-8">
                <div className="flex-1 space-y-6 lg:space-y-8">
                  ...
                  {/* Place the Day Plan card in main content */}
                  <DayPlanCard />
                          ...
                          {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((day, index) => {
                            ...
                            return (
                              <div key={day} className="flex flex-col items-center gap-3 flex-1 group cursor-pointer">
                                ...
                                <span className={`text-xs font-medium ${isToday ? 'text-orange-600 font-bold' : 'text-gray-500'}`}>{day}</span>
                              </div>
                            );
                          })}
                        </div>
                        <div className="flex justify-center">
                          <div className="text-sm text-gray-600 bg-gray-50 rounded-full px-4 py-2">🏆 You're having an amazing week!</div>
                        </div>
                      </div>
                    </div>
                      ...
                      <div className="flex flex-col sm:flex-row items-center gap-6 sm:gap-8">
                        <div className="relative w-32 h-32 sm:w-40 sm:h-40">
                          <svg className="w-full h-full transform -rotate-90">
                            ...
                            <circle cx="50%" cy="50%" r="45%" stroke="#14b8a6" strokeWidth="8" fill="transparent" strokeDasharray="283" strokeDashoffset="70" className="transition-all duration-1000" />
                          </svg>
                ...
                <div className="w-full xl:w-96 space-y-6">
                  <div className="bg-white rounded-3xl p-6 shadow-lg hover:shadow-xl transition-all duration-300">
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      {[
                        { label: 'Weight', value: data?.weight ?? '-', unit: 'kg', color: 'text-blue-600' },
                        { label: 'Height', value: data?.height ?? '-', unit: 'cm', color: 'text-green-600' },
                        { label: 'Age', value: data?.age ?? '-', unit: 'y', color: 'text-purple-600' },
                      ].map((stat, index) => (
                        <div key={index} className="text-center p-3 bg-gray-50 rounded-2xl hover:bg-gray-100 transition-colors">
                          <div className="text-xl font-bold text-gray-800">
                            {stat.value}
                            <span className={`text-sm ${stat.color} ml-1`}>{stat.unit}</span>
                          </div>
                  ...
                  <div className="bg-white rounded-3xl p-6 shadow-lg hover:shadow-xl transition-all duration-300">
                    <h3 className="font-bold text-gray-800 text-lg mb-4 flex items-center gap-2">Your Goals 🎯</h3>
                    <div className="space-y-4">
                      {[
